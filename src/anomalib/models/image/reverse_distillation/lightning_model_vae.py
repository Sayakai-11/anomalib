...
"""Reverse Distillation + VAE Lightning module (staged, PatchCore-style).
...
There is no memory bank. Inference scores patches by the closed-form NLL of
the VAE posterior mean (``1/2 ||mu||^2``).

Usage::

    from anomalib.models.image.reverse_distillation.lightning_model_vae import ReverseDistillationVAE
    from anomalib.engine import Engine
    from anomalib.data import MVTecAD

    datamodule = MVTecAD()
    model = ReverseDistillationVAE(backbone="wide_resnet50_2", latent_dim=512, rd_epochs=200)
    engine = Engine()
...
from .torch_model_vae import ReverseDistillationVAEModel

logger = logging.getLogger(__name__)


class ReverseDistillationVAE(ResidualCollectionMixin, AnomalibModule):
    """Reverse Distillation with a VAE trained after RD training completes.
    ...
    def __init__(
        ...
        self.rd_lr = rd_lr
        self.model = ReverseDistillationVAEModel(
            backbone=backbone,
    ...
    def on_train_epoch_end(self) -> None:
        """After the last Phase-1 epoch, run Phase 2 (VAE training) once."""
        ...
        self.model.train_vae(embeddings)
    ...
    def trainer_arguments(self) -> dict[str, Any]:
        return {
            ...
            # Only validate once, after Phase 2 has trained the VAE -- otherwise
            # Lightning's default per-epoch validation would run against an
            # untrained VAE during Phase 1.
