...
"""Reverse Distillation + VAE + flow Lightning module (staged, PatchCore-style).
...
Training is staged:
    ...
    Phase 2 -- once Phase 1 completes, the (now frozen) teacher-student
        residual is collected over the whole training set and the VAE is
        trained via ELBO on it.
    Phase 3 -- once Phase 2 completes, the (now frozen) VAE posterior mean is
        used to train a PatchFlow-style normalizing flow via NLL.

There is no memory bank. Inference uses PatchFlow's own NLL-based anomaly map
on the post-flow latent.

Usage::

    from anomalib.models.image.reverse_distillation.lightning_model_vae_flow import (
        ReverseDistillationVAEFlow,
    )
    from anomalib.engine import Engine
    from anomalib.data import MVTecAD

    datamodule = MVTecAD()
    model = ReverseDistillationVAEFlow(backbone="wide_resnet50_2", latent_dim=512, rd_epochs=200)
    engine = Engine()
...
from .torch_model_vae_flow import ReverseDistillationVAEFlowModel

logger = logging.getLogger(__name__)


class ReverseDistillationVAEFlow(ResidualCollectionMixin, AnomalibModule):
    """Reverse Distillation with a VAE-then-flow head trained after RD training completes.
    ...
    Args:
        ...
        vae_epochs: Number of Phase-2 (VAE) training epochs. Defaults to ``500``.
        vae_lr: Adam learning rate for the VAE. Defaults to ``1e-3``.
        vae_batch_size: Mini-batch size used during VAE training. Defaults to ``512``.
        flow_epochs: Number of Phase-3 (flow) training epochs. Defaults to ``50``.
    ...
    def __init__(
        ...
        self.rd_lr = rd_lr
        self.model = ReverseDistillationVAEFlowModel(
            backbone=backbone,
    ...
    def on_train_epoch_end(self) -> None:
        """After the last Phase-1 epoch, run Phase 2 (VAE) then Phase 3 (flow)."""
        if self.current_epoch != self.rd_epochs - 1:
            return
        logger.info("Reverse Distillation training complete -- collecting residuals for VAE training.")
        embeddings = self._collect_residuals(flatten=True)
        mu_all = self.model.train_vae(embeddings)
        logger.info("VAE training complete -- training flow on VAE posterior means.")
        self.model.train_flow(mu_all)
    ...
    def trainer_arguments(self) -> dict[str, Any]:
        return {
            ...
            # Only validate once, after Phases 2-3 have trained the VAE and flow --
            # otherwise Lightning's default per-epoch validation would run against
            # an untrained VAE/flow during Phase 1.
