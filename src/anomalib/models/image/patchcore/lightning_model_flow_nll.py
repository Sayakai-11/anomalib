...
"""Lightning module for PatchCore + RealNVP (no VAE) + ½‖z‖² scoring."""
...
from .torch_model_flow_nll import PatchcoreFlowNLLModel

logger = logging.getLogger(__name__)


class PatchcoreFlowNLLModule(MemoryBankMixin, AnomalibModule):
    """PatchCore Lightning module with RealNVP training and ½‖z‖² anomaly scoring.

    Training is identical to PatchcoreFlowModule (PCA + RealNVP via NLL, no
    VAE step). Inference uses the NLL of z under N(0, I) — no kNN.
    ...
    def __init__(
        ...
        self.model = PatchcoreFlowNLLModel(
            backbone=backbone,
    ...
    def fit(self) -> None:
        logger.info("Training RealNVP flow (no VAE) for NLL scoring.")
