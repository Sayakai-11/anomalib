...
"""Lightning module for PatchCore + VAE + Prior likelihood scoring."""
...
from .torch_model_vae_prior import PatchcoreVAEPriorModel

logger = logging.getLogger(__name__)


class PatchcoreVAEPriorModule(MemoryBankMixin, AnomalibModule):
    """PatchCore Lightning module using KL divergence from prior as anomaly score.

    Drop-in replacement for PatchcoreVAEModule.  Identical training API;
    only inference scoring changes from kNN to KL( q(z|x) || N(0,I) ).
    ...
    def __init__(
        ...
        self.model = PatchcoreVAEPriorModel(
            backbone=backbone,
    ...
    def fit(self) -> None:
        logger.info("Training VAE and building mu-bank (used for dtype/compat only).")
